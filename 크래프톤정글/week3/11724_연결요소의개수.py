...
class Graph:
    ...
    def find_all_vertex(self):  # 모든 정점을 반환
        all_vertices = list(self.adjacency_list.keys())
        return all_vertices

    def add_vertex(self, vertex):  # 정점추가
        if vertex not in self.adjacency_list:
            self.adjacency_list[vertex] = []
            return True
        return False

    def add_edge(self, vertex1, vertex2):  # 가중치 없는 간선추가
    ...
    def add_edge_with_cost(self, vertex1, vertex2, cost=1):  # 가중치 있는 간선 추가
        if not (vertex1 in self.adjacency_list and vertex2 in self.adjacency_list):
            return False
    ...
    def remove_vertex(self, vertex):  # 정점 제거 (가중치 있는 버전)
    ...
    def remove_edge(self, vertex1, vertex2):  # 간선 제거 (가중치 없는 버전)
        if not (vertex1 in self.adjacency_list and vertex2 in self.adjacency_list):
            return False
    ...
    def dfs_recursive(self, start):  # 재귀형 dfs
        if start == None:
...
while len(all_vertex) > 0:
    ...
    all_vertex = [v for v in all_vertex if v not in visted]  # while
