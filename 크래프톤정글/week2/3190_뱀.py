...
is_game_over = False
for _ in range(L):
    change_second, rocate = sys.stdin.readline().split()  # 바뀌는 시간과 방향
    change_second = int(change_second)
    if is_game_over:
        break

    while count < change_second:
        new_row = cur_loca[0] + direct[direct_idx][0]  # 현재 row + 방향 row
        new_col = cur_loca[1] + direct[direct_idx][1]  # 현재 col + 방향 col

        if new_col >= N or new_col < 0 or new_row >= N or new_row < 0:  # 넘어가면 탈락
            is_game_over = True
            break
        if visited[new_row][new_col] == True:  # 내 몸이 있으면 탈락
            is_game_over = True
            break
        if apple[new_row][new_col] == True:  # 사과가 있으면 길이를 1증가
    ...
    if rocate == "D":
        direct_idx = direct_idx + 1 if direct_idx < 3 else 0
    else:
        direct_idx = direct_idx - 1 if direct_idx > 1 else 3
